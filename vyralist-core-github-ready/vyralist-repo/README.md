...
```
Brand Posts Campaign → Creators Apply → Brief & Collaborate → Content Submitted → Reviewed & Approved → Creator Gets Paid
```
...
This repository contains the frontend source code for two of Vyralist's three production applications:

### `/brands-app`
Campaign management platform for brands. Built as a single-page application.
...
### 3. Add the env-config script to index.html
In both `brands-app/index.html` and `creator-app/index.html`, ensure this line appears **before** any app scripts:
```html
...
Run the SQL migration in `creator-app/MIGRATION_messaging_rls.sql` against your Supabase project to set up RLS policies for the messaging system.

---

## Business Model

Transaction margin — no subscriptions.
...
Year 1 Target: 50 brands × 3 campaigns/month × $90 avg margin = **$13,500 MRR**
...
| Pitch deck, business plan, financial projections complete | May 2026 |
...
This project is proprietary. Source code is shared publicly for portfolio and evaluation purposes only. No licence is granted to use, copy, or deploy this code commercially.
